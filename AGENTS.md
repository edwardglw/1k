...
- **Weight is fully optional in onboarding** (29 Apr 2026). `ScreenWeight.jsx` label reads "Current weight (optional)" with a clarifying subhead. The targetWeight dropdown only appears once weight is set (progressive disclosure). `canAdvance()` for step 3 is now `return true` — both fields are skippable. `ScreenPlanReady.jsx` hides the Weight & goal row entirely when no weight is set, matching the dashboard which already hides Weight Journey + calorie cards when `progress.startWeight` is null. Net: skipping weight cleanly removes the entire weight-tracking surface across onboarding → plan summary → dashboard.
...
```js
{
  ...
  bannerDismissedAt: null, // ms timestamp; t-shirt banner re-shows when a badge is earned after this
}
```

## Accessibility utilities

- **`src/lib/useFocusTrap.js`** — focus trap hook for modals/sheets. Pair with `role="dialog"` + `aria-modal="true"` on the same element. All 5 sheets (BenefitsModal, WeightLogSheet, SessionDoneSheet, ShareSheet, WeekFeelingSheet) use it.
- **Global `:focus-visible`** rule in `src/index.css` — 2px moss outline + 2px offset, `!important` overrides inline `outline: "none"` on inputs/selects.

## Sync error surfacing

`UserContext` exposes `syncError` (string|null) + `clearSyncError()`. Set on Firestore write failure in the profile/progress sync effects, cleared on next successful write. Rendered as `SyncErrorBanner` (apricot, top of viewport, `role="status" aria-live="polite"`) inside `AppRoutes` in `App.jsx` whenever authed and `syncError` is set.
...
- Weight is **fully optional** in onboarding (gate is now `return true` for step 3). If skipped, the dashboard Weight Journey card hides entirely, the plan summary's Weight & goal row hides, and the targetWeight dropdown in ScreenWeight is hidden until weight is set. Calorie allowance also depends on weight, so users who skip won't see an allowance card either.
