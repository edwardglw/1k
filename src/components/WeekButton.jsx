...
export default function WeekButton({ num, status, isGoal, isSelected, onClick }) {
  ...
  if (isGoal) {
    ...
    bg = isSelected ? T.color.white : T.color.apricot;
    border = T.color.apricot;
    textColor = isSelected ? T.color.apricot : T.color.white;
    shadow = isSelected ? `0 4px 14px ${T.color.apricot}44` : `0 2px 6px ${T.color.apricot}44, inset 0 1px 0 rgba(255,255,255,0.25)`;
  } else if (isCurrent) {
    ...
    shadow = isSelected
      ...
      : `0 4px 14px ${T.color.moss}55, inset 0 1px 0 rgba(255,255,255,0.2)`;
    animation = isSelected ? undefined : "weekPulse 2s ease-in-out infinite";
  } else {
    // Upcoming — outline only, clearly reachable but de-emphasised
    bg = isSelected ? T.color.charcoalMuted : "transparent";
    border = isSelected ? T.color.charcoalMuted : T.color.ivoryDark;
    textColor = isSelected ? T.color.white : T.color.charcoalMuted;
    shadow = isSelected ? "0 4px 12px rgba(59,63,58,0.28)" : "none";
  }
  ...
  return (
    <>
      <style>{`
        @keyframes weekPulse {
          0%, 100% { box-shadow: 0 4px 14px ${T.color.moss}55, 0 0 0 0 ${T.color.moss}44; }
          50% { box-shadow: 0 4px 14px ${T.color.moss}55, 0 0 0 6px ${T.color.moss}00; }
        }
      `}</style>
      <button
        onClick={onClick}
      ...
      >
        ...
        {isDone && !isGoal && (
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center",
            border: `2px solid ${T.color.apricot}`,
            boxShadow: `0 1px 3px ${T.color.apricot}55`,
          }}>
            <Icon type="star" size={7} color={T.color.apricot} />
          </div>
        )}
